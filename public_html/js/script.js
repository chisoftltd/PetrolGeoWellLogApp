...
function eggYolk() {
	var text = " :  Gamma ray index ";
	if (portions == 1) {
		text = text;
	} else {
		text += "s";
	}
        gri = (portions - 23)/(100-23);
	document.getElementById("egg_yolk").innerHTML = gri.toFixed(2) + text;
        milk();
        sugar();
        heavy_cream();
}
...
function milk() {
	document.getElementById("milk").innerHTML = (0.083 *(Math.pow(2, (3.7 * gri) - 1.0))).toFixed(2) + " : for younger rocks";
}


function sugar() {
	document.getElementById("sugar").innerHTML = (0.33 *(Math.pow(2, (2 * gri) - 1.0))).toFixed(2) + " : for older rocks";
}

function heavy_cream() {
	document.getElementById("heavy_cream").innerHTML = gri.toFixed(2) + " : rock age unknown";
}
